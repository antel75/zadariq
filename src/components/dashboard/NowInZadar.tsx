...
function useSportsForNow() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // 2) Today (next 24h upcoming, non-stale)
    ...
    for (const ev of sportsEvents.events) {
      ...
      if (tier === 'live') {
        candidates.push({
          ...
          action: () => ev.link_url ? window.open(ev.link_url, '_blank') : window.open('https://www.sofascore.com/', '_blank'),
          priority: 120,
          isActionable: true,
          pinFirst: true,
        });
        ...
        candidates.push({
          ...
          label: `${emoji} ${isF1 ? t('happening.nextRace') : teamLabel}`,
