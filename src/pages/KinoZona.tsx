...
export default function KinoZona() {
  ...
  const { data: movies, isLoading: moviesLoading } = useQuery({
    queryKey: ['kino-zona-movies'],
    queryFn: async () => {
      const { data, error } = await supabase.from('kino_zona_movies').select('*').order('title');
  ...
  const { data: screenings, isLoading: screeningsLoading } = useQuery({
    queryKey: ['kino-zona-screenings'],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('kino_zona_screenings')
        .select('*')
  ...
  const filteredMovies = useMemo(() => {
    ...
    if (dayFilter === 'today') dateFilter = (d) => d === todayStr;
    else if (dayFilter === 'tomorrow') dateFilter = (d) => d === tomorrowStr;
    else dateFilter = (d) => d >= todayStr && d <= weekEndStr;

    const filtered = screenings.filter(s => dateFilter(s.screening_date));
    ...
    return movies
      ...
      .map(m => ({
        ...m,
        screenings: filtered.filter(s => s.movie_id === m.id),
      }));
  ...
  const groupScreeningsByDate = (movieScreenings: typeof screenings) => {
    if (!movieScreenings) return [];
    const groups: Record<string, typeof movieScreenings> = {};
    ...
    <div className="min-h-screen bg-background pb-8">
      <div className="bg-primary text-primary-foreground px-4 pt-12 pb-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h1 className="text-xl font-bold">🎬 Kino Zona</h1>
            <p className="text-xs text-primary-foreground/70">Kneževa palača, Stari grad</p>
          </div>
        </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                      {movie.genre && (
                        <span className="text-[10px] px-2 py-0.5 rounded-full bg-accent/10 text-accent font-medium">{movie.genre}</span>
                      )}
                      {movie.duration && (
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {movie.screenings.slice(0, 5).map(s => (
                        <span key={s.id} className="text-xs font-bold bg-primary/10 text-primary px-2 py-0.5 rounded-md">
                ...
                {isExpanded && (
                  <div className="border-t border-border px-3 py-3 space-y-3 bg-muted/30">
                    ...
                    {screeningGroups.map(([date, dateScreenings]) => (
                      <div key={date}>
                        <p className="text-[11px] font-semibold text-foreground/70 mb-1">{formatDateLabel(date)}</p>
                        <div className="flex flex-wrap gap-2">
                          {dateScreenings.map(s => (
                            <div key={s.id} className="text-sm font-bold text-primary bg-primary/10 px-3 py-1 rounded-lg">
                              {s.screening_time.substring(0, 5)}
                            </div>
                          ))}
                        </div>
